...
const { chalkError, chalkSuccess, chalkWarning } = require('./utils/index')

/** 启动doc项目 */
exports.start = function(options) {
  const configFile = path.join(process.cwd(), options.config || 'doc.config.js')
  ...
  new WebpackDevServer(
    {
      ...
      static: {
        directory: docConfig.themeDir, // 设置静态文件实际目录
        publicPath: '/',
      },
      port: docConfig.port,
      host: '0.0.0.0',
...
exports.build = function(options) {
  ...
  const webpackProdConfig = genWebpackProdConfig({
    entry: {
  ...
  if (docConfig.dll) {
    ...
    webpackProdConfig.plugins.push(
      new webpack.DllReferencePlugin({
        ...
        manifest: resolveProject(`public/dll/${docConfig.dll.name}-manifest.json`),
      }),
      new HtmlWebpackPlugin({
        filename: htmlFileName,
        template: docConfig.htmlTemplate,
        chunks: [docConfig.entryName],
        favicon: docConfig.favicon ? resolveProject(docConfig.favicon) : '',
      }),
